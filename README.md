...
本项目及全部改造代码以 **GPL-3.0** 发布，与原项目一致。许可证全文见上游自带的 [LICENCE](./LICENCE)（英式拼写）。

> 勘误说明：上游仓库自第一个 commit（`d8fea11`）起即附带 GPL-3.0 全文，文件名为 `LICENCE`（英式拼写）。GitHub API 的 `license.path` 未能识别该文件（返回 null），导致本仓库维护者一度误判"上游无许可证文件"并重复添加了一份 `LICENSE`，该重复文件已删除，特此更正。
